...
func TestMigration0011_RoundTrip(t *testing.T) {
	...
	// Migrations now sit on top of 000011 (000012 RBAC, 000013 drop workspaces,
	// 000014 sessions). Migrate down to exactly 000011 so this test exercises the
	// 000011 down→up round-trip with 000011 as the effective head — the schema
	// state it was written to assert against. Pinned to an absolute version so
	// later migrations don't shift it. 000013.down recreates workspaces and
	// re-adds workspace_id along the way.
	if err := m.Migrate(11); err != nil {
		t.Fatalf("migrating down to 000011: %v", err)
	}
